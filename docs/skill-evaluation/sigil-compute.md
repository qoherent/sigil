...
| Non-conforming child gets no repair (9a–9c) | A prose child and a malformed-rows child reached ingest verbatim — no repair, no stripped prose, no second attempt — and each was refused with no state. The truncated-artifact case exercised the tool's parser refusal on an observer-crafted truncation; the fixture's host-side variant (a truncated child response with no completion statement, stopped before ingest) is recorded as unobserved, partially covered by the interrupted-delegation case. | [Failure records](evidence/sigil-compute/failure-cases.json) |
