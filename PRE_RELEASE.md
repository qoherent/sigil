...
Sigil 0.1.0 is a work-in-progress pre-production release of the language,
platform, and agent workflow. It does not claim production readiness or a
stable 1.0 contract.

Included contracts:

- Sigil Language 0.1.0;
- `.sigil/config.json` schema 0.1.0;
- `@qoherent/sigil-core` 0.1.0;
- `@qoherent/sigil` 0.1.0;
- standalone Codex Sigil skill 0.1.0 with implementation-component discovery
  and coverage review;
- language, configuration, API, migration, workflow, and compatibility
  documentation.

The initial 0.x scope includes `@qoherent/sigil-lsp` 0.1.0 as the shared
editor-neutral language-server boundary over `@qoherent/sigil-core`.

The initial 0.x scope also includes the Sigil VS Code extension 0.1.0 as the
...
Version 0.1.0 intentionally excludes anchors and `sigil-indexer` implementation,
