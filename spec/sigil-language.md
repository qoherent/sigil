...
| Contract | One architectural question answered about a component or Trait | `goal`, `interface`, `state`, `logic`, `constraints`, `decisions`, `cases` |
...
| Trait | A reusable architectural contract to which multiple components may conform; it has no `logic` of its own | `trait Name { ... }` |
| Conformance | A component declares conformance to Traits; their obligations collect without override or order-based precedence | `component C is Trait1, Trait2 { ... }` |
| Tag import | File-scoped selection of Tags, including Concepts, from a component | `@path/file.sigil from Component import { a, b }` |
| Trait import | File-scoped selection of Traits from a `.sigil` source | `@path/file.sigil import { Trait1, Trait2 }` |
...
`goal` and `interface` must be nonempty in every component and Trait. Other
...
- Tag names are case-sensitive, exact, and not whitespace/Unicode-normalized.
  Define a local Inline Tag once with `*name*`; use its bare name afterward. A
  definition is recognized only with the required outer boundary characters;
  `* name *` and fenced payloads do not define one.
...
- A component or Trait owns its local Tag vocabulary. A bare Tag from another
  declaration is unavailable until imported; declaring `is Trait` imports the
  Trait, not the Tags owned by unrelated components it references.
...
- Traits require `goal` and `interface`, may contain `state`, `constraints`,
  `decisions`, and `cases`, and cannot contain `logic`. Trait-to-Trait `is` is
  not composition. A component's `is` names imported Traits only.
- Trait `interface`, `state`, `constraints`, and `cases` contribute collectively
  to the conforming component; they do not override one another. Trait
  `decisions` remain rationale. A Concept block in component `logic` supplies
  Trait evidence only when its heading exactly matches the Trait name and the
  component declares `is Trait`; importing a Trait alone is not conformance. A
  differently named Concept is an ordinary grouped concern.

## Imports

Import statements are top-level, before component and Trait declarations, and
are file-scoped: every declaration in that source sees the same imports. Tag
imports select named Tags from the stated owning component source. Trait imports
select Traits from a `.sigil` source without `from Component`; `from Component`
never names a Trait. Imports name declarations in `.sigil` files, never
Markdown guides or evaluation notes. A selected Tag must be referenced in prose;
...
qualified references, re-exports, or inherited provider imports. Importing a
name supplies vocabulary or conformance; describe runtime interactions in
`interface`/`logic`.
...
decisions, and conformed Traits aligned with the revised behavior. Moving a
source requires updating imports and relative links.
...
valid Tag and Trait ownership; elegant representations; available provider/link
context; explicit unresolved choices; and agreement among contracts, Cases,
links, and conformed Traits. Optional contracts, Tags, Concepts, links, and
detail may be absent. Report actual checks.
