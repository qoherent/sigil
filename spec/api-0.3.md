...
and must declare at least one local component. An index without a local
component produces `SIGIL_MODULE_WITHOUT_COMPONENT` while retaining its partial
document.
...
Sigil source/config problems are diagnostics. Filesystem inability propagates
to the host.
...
`@qoherent/sigil-lsp` 0.3.0 implements Language Server Protocol 3.18 over
